...
FLAI is a self-hosted multimodal AI assistant running on a **single consumer NVIDIA GPU (8/12/16+ GB)**. It orchestrates multiple models (multimodal, reasoning, embedding, SD, LTX-Video) through a strict GPU queue with VRAM-aware scheduling.
  ...
  - **Background tasks:** `app/tasks/` — `dry_load.py` (dry-load after admin save; also scheduled for context-only changes, with a context-aware rollback that restores `context_length`, not the fallback model), `health_monitor.py` (crash-loop watchdog; **skips its whole tick while `ResourceManager.is_gpu_busy()` is active** — SD, video, or an `ensure_vram_for` unload+wait cycle — because a health check spawned mid-wait respawns the model being unloaded and starves the VRAM wait). Fact extraction runs as background thread (CPU-only, rule-based via `app/slm_rules.py`). Fact merge runs on background queue (CPU-only, no LLM). Both excluded from queue status display and user counter. SLM profiles are tied to user accounts: `userdb.delete_user()` GDPR-erases and removes the user's daemon profile via the wrapper's `/delete-profile` route (non-blocking, warning on failure).
  - **LLM client:** `app/llamacpp_client.py` with `DirectLlamaBackend` and `LlamaSwapBackend`. Both `call()` and `chat()` accept `temperature` parameter. Router classification uses hardcoded `temperature=0.1`. **Router action-time & session micro-context (v12.1):** the router decides the action time first (`prompts/{ru,en}/base_text.template` «ACTION TIME» section): past-tense requests («we watched», «you showed earlier», «yesterday's snapshots») route to `[-HISTORY-]`, and the action categories (incl. camera — only a request to see the room state NOW) never capture questions about the past. `process_message()` accepts `recent_context` built by `BaseModule.build_router_context()`: the last `ROUTER_CONTEXT_MESSAGES` real messages via `get_session_recent_history()` in `app/db.py` (newest tail, generation-marker pair filter, current message excluded, each ≤`ROUTER_CONTEXT_MSG_CHARS` chars) plus up to `ROUTER_SLM_FACTS` SLM long-term facts; `_route_text_action()` in `app/queue.py` feeds it per request. Env: `ROUTER_CONTEXT_MESSAGES` (6) / `ROUTER_CONTEXT_MSG_CHARS` (240) / `ROUTER_SLM_FACTS` (2). `_translate_llama_swap_error()` translates llama-swap errors to user language. `_strip_generic_reasoning()` threshold `>=2` markers (synced with JS client), overbroad Russian patterns removed to prevent false positives. **Repetition-loop detector:** streaming content goes through `_LoopGuard` (hold-back 1500 chars, `_repetition_cutoff` detects ≥3 equal-gap repetitions of the last-200-char block); both backends route in-loop content AND end-of-stream remainders (`_stream_buffer` + `_reasoning_stream` + `_loop_guard` flushes, in that order) through the guard so short answers are never dropped or reordered. Safety net `strip_repetition_loop()` in `app/queue.py:_process_reasoning_request()` cuts loops missed in streaming. Transient HTTP 502 from llama-swap (JIT load under reasoning `ttl=1s`) is retried up to 3 attempts in `chat_stream()` for the reasoning model, and `_process_reasoning_request()` retries once more and logs the body when the collected stream is a 502/«HTTP error» marker.
  ...
  - **Kokoro cold-start warmup:** `services/kokoro/app.py:_warmup_ru()` runs one background ru synthesis at startup (`KOKORO_WARMUP_G2P=1`), so the first ru phrase takes ~2 s instead of ~56 s. RUAccent G2P worker is unloaded after `KOKORO_G2P_IDLE_TIMEOUT` (default 300, `0` = never). Client timeout `KOKORO_TIMEOUT=60`. The frontend sends `gender` explicitly in every synthesize payload (stale session cookie after `/set-voice-gender` must not change the voice).
...
# 1. GPU Queue — Strict Serialization
  ...
  - **RAG retrieval never drops a user document (file coverage):** the three reasoning-context builders call `rag.search(user_id, query, top_k=20, file_coverage=True)` — for every indexed document of the user missing from the semantic top-k, its best chunk is fetched (per-`doc_id` filter, `limit=1`) and appended (bounded by `coverage_max_files`, default 6) marked with `payload['coverage']=True`; coverage chunks bypass the relevance-threshold filter (`_process_rag_task`/`_process_rag_task_stream`) while the fixed top-15 semantic cap stays separate, so a low-scoring contract still reaches the reasoning model instead of silently vanishing from the context.
...
# Security
  ...
  - **HTML preview (`/api/html-preview/<message_id>`):** the chat ▶ button opens the message's ```html block server-side (not via a blob: URL — blob pages inherit the chat's strict CSP and CDN imports render a blank screen). The button resolves the message id from the element's `data-message-id`, which live streamed messages get in `finalizeStreamedMessage()` from `result.message_id` — that requires `save_message()` to return the real id via `INSERT … RETURNING id` (**never use `cursor.lastrowid`: it is always 0 on PostgreSQL/psycopg2**, a SQLite-ism that once made every fresh preview alert «unavailable» until F5). `onMessageNew()` must not pre-add the message id to `displayedMessageIds` (displayMessage owns dedup and refuses ids already in the set) and skips rendering while a stream is active for the session (the finalize path renders the same message). The endpoint validates login + session ownership and sets its own relaxed CSP (`X-Own-CSP: 1` marker skips the global `after_request` CSP override); CDN origins are whitelisted only on that response and `'unsafe-eval'` is allowed (legacy UMD Three.js bundles from cdnjs self-execute via `Function()`/`eval` — without it `THREE.Scene` never exists and the preview is blank). **Import-map rescue:** generated Three.js pages sometimes skip the `<script type="importmap">` and import addon modules by absolute CDN URL (`unpkg`/`jsdelivr`); three r160+ addons internally import the bare `three` specifier, so without the map the module graph fails to link and the preview is blank. `_ensure_three_importmap()` in `app/routes/messages.py` detects the missing map, injects one serialized with `json.dumps()` (hand-built f-strings once collapsed `}}` into a single `}` and Chrome rejected the map as invalid JSON — a regression test now `json.loads` the injected map; version from the first CDN URL, default 0.160.0) and rewrites build/addons imports to the bare `three`/`three/addons/` aliases; pages that already use an import map are untouched.
