...
# Security
  ...
  - **HTML preview (`/api/html-preview/<message_id>`):** the chat ▶ button opens the message's ```html block server-side (not via a blob: URL — blob pages inherit the chat's strict CSP and CDN imports render a blank screen). The endpoint validates login + session ownership and sets its own relaxed CSP (`X-Own-CSP: 1` marker skips the global `after_request` CSP override); CDN origins are whitelisted only on that response. **Import-map rescue:** generated Three.js pages sometimes skip the `<script type="importmap">` and import addon modules by absolute CDN URL (`unpkg`/`jsdelivr`); three r160+ addons internally import the bare `three` specifier, so without the map the module graph fails to link and the preview is blank. `_ensure_three_importmap()` in `app/routes/messages.py` detects the missing map, injects one (version from the first CDN URL, default 0.160.0) and rewrites build/addons imports to the bare `three`/`three/addons/` aliases; pages that already use an import map are untouched.
