...
- **Enhanced web search results quality** — Three fixes to make news queries actually return useful content: (1) `_fetch_page_content()` runs via `ThreadPoolExecutor` (max 3 concurrent) for all results with short snippets (<300 chars), not just empty ones; (2) Instructions softened from «ИСПОЛЬЗУЙ ТОЛЬКО ЭТИ ДАННЫЕ» to «используй эти данные как основной источник» in both `modules/base.py` heading and `prompts/{ru,en}/reasoning.template`, allowing the model to supplement with its own knowledge when search data is insufficient; (3) SearXNG initially used `categories=general,news` for article-level results, but this was later removed as it caused DuckDuckGo rate limiting on SearXNG (CAPTCHA errors).
