...
- **Auto-fit context window at deployment (GPU seed)** — instead of one hardcoded context size for every GPU, `app/database.py` now auto-fits each model's `context_length` to the actual hardware. `_autofit_context()` reads the real GGUF metadata straight from disk (`arch_max_ctx`, file size, `expert_count`, `supports_mtp`, mmproj size) plus measured system RAM and GPU VRAM, then picks the largest tier goal (24 GB → 32768/32768, 16 GB → 32768/24576, 8–12 GB → 16384/16384, unknown VRAM → 32768/24576) that fits the model body + KV cache + mmproj inside the 85% VRAM / 70% RAM budget, stepping down through 32768→24576→16384→8192. If even the smallest step cannot fit fully (the model body itself is already bigger than VRAM), the goal context is returned unchanged and the existing per-layer `n_gpu_layers` degradation handles offload. Context migrations in `init_db` are now VRAM-aware too: they only raise a module's window when the auto-fit target is larger than the current value, so a smaller GPU never gets a window it cannot hold. 16 tests in `tests/test_autofit_context.py`.
- **CPU-only stack switched to lightweight models** — `deploy.sh` / `deploy-ru.sh` CPU branch now downloads gpt-oss-20b-mxfp4 (native MXFP4, CPU-friendly per llama.cpp) as the reasoning model instead of Qwen3.6-35B, and Qwen3VL-4B (~2.5 GB + mmproj) as the multimodal model instead of Qwen3VL-8B, with the CPU context window set to 8192. Vision, RAG, and SLM capabilities are preserved; the largest resident model on CPU drops to ~11 GB. `app/llama_swap_config.py` sets `--reasoning-format auto` only for gpt-oss architectures (the codeless thinking split), keeping the explicit format for Qwen/DeepSeek and avoiding server startup warnings.
- **Clipboard image paste in chat** — images can now be attached by pasting directly into the message input (Ctrl+V / mobile "Paste"). Clipboard priority: text-only → normal text paste; image-only → attached as a file; both text and image → the image wins (copying a picture from a browser often carries an HTML/text snippet alongside the bitmap). Pasted blobs (which have no filename) are wrapped into a generated `File` named `pasted_YYYYMMDD_HHMMSS.<ext>` (extension from MIME type, `jpeg` → `jpg`) and flow through the existing file-attach preview and `FormData` upload path — no backend changes. Handler in `app/static/js/chat-init.js`.
- **Fresh GPU deployments failed: llama-swap rejected the deploy.sh seed config** — `generate_llama_swap_config()` in `deploy.sh`/`deploy-ru.sh` wrote the bootstrap `llama-swap.yaml` in the old list format (`models: \n  - model: ...`), but llama-swap ≥ v243 (the floating `:cuda` tag now resolves to v255) requires `models` to be a map — llama-swap died on startup with `cannot unmarshal !!seq into map[string]config.modelMacroConfig`, and since the web app had not started yet nothing regenerated the file. Fixes: the seed config now uses the current map format (`logLevel` / `startPort` / `models.multimodal.cmd`, verified against llama-swap v255 in a container); the llama-swap image is pinned (`v255-cuda-b10991`, `v255-cuda13-b10991`, `v255-cpu-b10991`) in `deploy.sh`, `deploy-ru.sh` and both compose files so upstream format changes no longer break deployments silently.
- **Whisper crashed on hardened hosts: "cannot enable executable stack"** — `libctranslate2` in the `onerahmet/openai-whisper-asr-webservice:v1.9.1` image is built with an executable-stack (`PT_GNU_STACK`) flag; hosts with a hardened AppArmor policy (Ubuntu 23.10+/24.04) deny execstack and the container dies on import (`Permission denied`), leaving ASR unavailable (AudioModule retries 5× then gives up per request). Fixed by adding `security_opt: [apparmor=unconfined]` to the whisper service in both compose files.
- **Fresh clones had no translations (footer, labels, language switch)** — `.mo` files are gitignored, the `./translations:/app/translations` bind-mount hides the ones compiled into the image, and the old deploy step compiled them only after the web app had already started (Flask-Babel reads catalogs at startup, so the restart was still required). `deploy.sh`/`deploy-ru.sh` now compile `.mo` before the first `up -d` (host `pybabel` when available, otherwise inside the container followed by a `flai-web` restart).
- **Reasoning stream buffers the whole answer on CPU (adaptive hold-back)** — the repetition-loop `_LoopGuard` (v11.4) holds back the last 1500 chars of a reasoning stream so a detected loop can be retracted, but on a CPU box (~1.6 tok/s on lenovo-book) a typical 600–1400 char answer never exceeded that window during generation: zero tokens were emitted mid-stream and the entire answer appeared as one block after generation finished (verified in DB: message id=60, 1395 chars / 543 tokens / 5.8 min). Fix: `_LoopGuard` and `_ReasoningStreamFilter` now shrink their hold adaptively on slow streams — after 40 `feed()` calls (many small feeds = slow generation) the hold linearly shrinks by 30 chars per extra feed, flooring at 300 chars (`_HOLD_FLOOR`). Fast GPU streams (few large chunks) keep the full 1500-char hold; loop detection still requires ≥600 chars of history and the post-stream safety net (`strip_repetition_loop()`) cuts anything the smaller hold-back lets through. On the CPU box the first visible token now appears ~320 chars into the answer and the rest streams progressively. 5 tests in `TestAdaptiveHold`.

### 🔧 Fixes (since v11.3 release)

- **Uniform task progress for all chat tool calls** — the `tool_call` indicator is now complemented by a real `task_progress` stage for every tool the chat model invokes: `get_current_time` → `getting_time` («🕐 Получаю время...»), `calculator` → `calculating` («🔢 Вычисляю...»), `time_calc` → `calculating_date` («📅 Рассчитываю дату...»), `web_search` → `searching_web` («🌐 Ищу в интернете...»), `rag_search` → `searching_documents` («📚 Ищу в документах...»), `camera_snapshot` → `capturing_snapshot` («📹 Делаю снимок...»). Previously only router-dispatched search paths published `task_progress` stages, so a weather/news question classified as a simple chat query showed no «Ищу в интернете...» status even when the model decided to call the `web_search` tool. Stage labels were added to `events.js` `STAGE_LABEL_KEYS`, `chat.html` translations, and both `.po` catalogs.
- **Informative task progress stages** — between request submission and the first answer token the user previously saw only «Загружаю модель рассуждений...» (which actually covered VRAM cleanup, model load, prompt build AND the model's entire silent `reasoning_content` thinking phase — up to 60+ s with a ticking counter). New stages published through the existing `task_progress` SSE channel: `routing` («🔍 Анализирую запрос...», emitted in `_route_text_action()` and covering the possible multimodal cold start), `searching_documents` («📚 Ищу в документах...», emitted before `rag.search()` in both RAG requeue paths and the reasoning-request fallback), `searching_web` («🌐 Ищу в интернете...», emitted in `_process_search_task()`), and `reasoning_thinking` («🤔 Обдумываю ответ...», emitted via a new `status_callback` that both LLM backends fire right after `POST /v1/chat/completions` returns — i.e. when the model is loaded and generation starts). Found-count labels («Найдено %s результатов/фрагментов, готовлю ответ...») are published with `results`/`chunks` counters; each progress indicator ticks elapsed seconds per phase («Ищу в интернете... (7 с)») and the frontend keeps exactly one progress element per session (phase chains cross task ids: fast-worker routing/search → requeued reasoning task). Stages are only published for real tasks (legacy non-streaming path keeps silence to avoid stale synthetic-task indicators).
- **Streaming repetition-loop detector** — reasoning models under a high `repeat_penalty` (see below) or on long web-search contexts can loop in `content` or emit empty/truncated answers. New `app/llamacpp_client.py` machinery: `_repetition_cutoff(text)` detects textual loops (scan window 4096 chars, block = last 200 chars, cycle = ≥3 equal-gap repetitions) and `_LoopGuard` (hold-back 1500 chars) delays stream tail emission so a detected loop can be cut before it reaches the user; both streaming backends feed their content through it, `_LoopGuard` is shared/attached per client. Safety net `strip_repetition_loop()` also runs on the assembled non-streaming response in `_process_reasoning_task()` (loops detected there retry once, mirroring the empty-output retry).
- **Loop-guard hold-back tail was silently dropped in `LlamaSwapBackend.chat_stream`** — on a clean stream end the reasoning-filter flush ran but `_loop_guard.flush()` was missing (the direct backend had it), so the up-to-1500-char hold-back tail never reached the user; for answers shorter than the hold-back the *entire* answer was lost (observed symptom: a 30-char fragment «нды из предоставленных данных:» stored as the full assistant message). Additionally, the end-of-stream `_stream_buffer` remainder was yielded **bypassing** the loop guard in both backends, which reordered/lost content for short answers. Both flush paths now route through `_loop_guard.feed()` before yielding, mirroring the in-loop path; regression tests in `TestChatStreamReasoningTailFlush` (short answer arrives in full, long answer keeps its tail).
- **Web search: relative date words are normalized to absolute dates** — search engines have no notion of "yesterday", so «Какие ИТ новости были вчера в мире?» returned landing pages of news sections instead of dated articles. New `enhance_query_with_date()` in `modules/search.py` appends the resolved date in the user's timezone (from `app.config["TIMEZONE"]`) after the relative word — «вчера (14 сентября 2026)» / "yesterday (September 14, 2026)" — handling «позавчера/вчера/сегодня» and English equivalents; queries without relative date words are untouched (query normalization, not routing). 8 tests in `TestEnhanceQueryWithDate`.
...
- **Plain-text chain-of-thought leaking into reasoning streams** — `app/llamacpp_client.py` now strips generic chain-of-thought leaked into `content` from reasoning streams (mirrors the queue-level `_strip_generic_reasoning()` safety net for non-streaming paths).
- **CPU video estimate now includes VAE decode time** — the first E2E run (lenovo-book, 256×192×57) completed denoising in 16:31 (matching the 24 000 voxels/s calibration) but spent ~43 min on VAE decode + mp4 assembly — the 300 s fixed overhead underestimated this by ~9×, so the smallest format still exceeded the 3600 s client timeout. `estimate_cpu_generation_time_s()` now adds `CPU_VAE_SECONDS_PER_FRAME=45` per frame; with the default 3060 s budget the planner now honestly refuses micro-formats that cannot finish in time (test `test_plan_time_budget_refuses_when_smallest_exceeds`), and `LTX_VIDEO_TIMEOUT` was raised to 7200 s on the test rig so a 57-frame clip completes end-to-end.
- **CPU video planner reports the binding constraint** — when no CPU format fits both the RAM and wall-clock budgets, the error now names which constraint failed and by how much (instead of a generic "does not fit").
- **CPU compose: ltxvideo gunicorn timeout** — raised above the 7200 s client timeout so a full-length 57-frame CPU clip is not killed mid-generation by the web worker.
- **Reasoning stream debug log** — `chat_stream` logs the streamed `reasoning_content` length for post-incident analysis.
