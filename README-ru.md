...
| v8.3 (Новое) | Примечания |
|--------------|------------|
| **Переключатель стилей ответов** | Выпадающее меню в шапке чата для выбора тона ответа: нейтральный, академический, деловой, дружеский, весёлый. |
| **Штраф за повтор** | Новый параметр `repeat_penalty` (диапазон 1.0–2.0) предотвращает зацикливание/повторы в ответах модели. Настраивается для каждой модели в админ-панели. По умолчанию: chat 1.1, reasoning 1.15, multimodal 1.1. |
| **PostgreSQL 18** | Обновление с 16 до 18 без потери данных (pg_dump + pg_restore). Точка монтирования изменена на `/var/lib/postgresql` (требование 18+). |
| **Форматирование служебных префиксов** | Сообщения о распознавании речи, снимках с камеры, генерации и редактировании изображений отображают префикс жирным шрифтом. Префиксы исключены из озвучивания TTS и копирования в буфер обмена. |
| **SSE-доставка в реальном времени** | Результаты очереди и новые сообщения доставляются через Server-Sent Events (Redis pub/sub) вместо HTTP-пуллинга. Мгновенные обновления, меньше трафика, нет задержек опроса. |
