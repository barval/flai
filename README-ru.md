...
| `[-HISTORY-]` | **Поиск в истории** | SQL `ILIKE` по прошлым сообщениям пользователя | Фрагменты истории + SLM + сводка + история |
...
| `none` (нет маркера) | **Чат с инструментами** | Мультимодальная модель + нативный tool calling | Результаты инструментов (calc, time, web_search, rag_search, camera) + SLM + история |
...
- Инструменты в режиме `none` (калькулятор, время, веб-поиск, RAG, камера) выполняются на fast worker и стримят прогресс вживую.
