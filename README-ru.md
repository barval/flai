...
| 📊 **VRAM-калькулятор** | Новый эндпоинт `/admin/api/model-estimate` оценивает VRAM для каждой модели (веса + KV cache + compute). Интерфейс настройки модели с авто-расчётом `n_gpu_layers` и слайдером |
| 🎨 **Offload-система для SD** | Рефакторинг `sd_wrapper.py`: 4 уровня выгрузки VRAM (0=полный GPU → 3=полный CPU), прогрессивная деградация при OOM. Проверка запаса VRAM (500MB) перед генерацией |
| ⚡ **Скорость токенов в реальном времени** | Оценка токенов/сек во время стриминга + финальная скорость в заголовке сообщения. `completion_tokens` сохраняется в БД и передаётся через SSE |
| 🔧 **Исправление кеша конфигурации моделей** | TTL-кеш заменён на версионирование по `updated_at` — устраняет рассинхронизацию между воркерами gunicorn при `workers=2` |
| 🐛 **Исправление отображения архитектуры** | Декодирование numpy byte-string (`[113 119 101 110 51]` → `qwen3`) в админ-панели. Regex обрабатывает варианты с ведущим пробелом |
| 🗃️ **Расширение метаданных GGUF** | `parameter_count`, `head_count`, `head_count_kv`, `key_length`, `value_length` — сканируются и сохраняются в БД |
