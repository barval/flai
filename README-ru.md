...
|-------------|------------|
| **Повторная попытка модели рассуждений** | Если модель рассуждений (gpt-oss-20b) вернула только блок размышлений (пустой ответ после срезания тегов), задача автоматически повторяется один раз. При повторе модель уже загружена, поэтому вторая попытка быстрая. Устраняет случайные ошибки «Не удалось получить ответ от модели рассуждений», вызванные холодной моделью, выдавшей анализ без финального ответа. Без повтора при отмене задачи или реальных ошибках модели. |
...
- **Повторная попытка модели рассуждений** — если модель рассуждений вернула только размышления (без финального ответа), задача автоматически повторяется один раз; устраняет случайные ошибки «Не удалось получить ответ от модели рассуждений» при холодных загрузках модели
