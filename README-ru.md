...
ПЛИИ v8.1 — модульное Flask-приложение, оркестрирующее сервисы на экосистеме llama.cpp.

### Что нового в v8.1

| v8.1 (Новое) | Примечания |
|--------------|------------|
| Поддержка llama-swap | Динамическое управление моделями и оптимизация использования видеопамяти |
| Потоковый синтез речи Piper | Большие тексты разбиваются на предложения и воспроизводятся без задержек |
| Оптимизация редактирования изображений | Автоматическое уменьшение изображений для Flux.2 Klein 4B, чтобы вписаться в 16 ГБ VRAM |
| Предиктивная выгрузка моделей | Очередь предсказывает следующую нужную модель и выгружает текущую для освобождения VRAM |
| Резервное копирование | Встроенная система создания и восстановления резервных копий (полных и только пользователей) |
| Интерфейс настройки чанков | Панель администратора для изменения размера чанков, перекрытия, стратегии и порогов RAG |
| Circuit Breaker | Защита от каскадных сбоев сервисов llama.cpp и sd.cpp |
| Менеджер ресурсов | Адаптивное управление GPU/CPU/RAM для предотвращения ошибок нехватки памяти |
...
3. Для каждого модуля (Чат, Рассуждения, Мультимодальность, Эмбеддинг):
   - Нажмите 🔄 **Обновить** — модели загружаются автоматически через llama-swap
...
2. Для каждого модуля (Чат, Рассуждения, Мультимодальность, Эмбеддинг):
   - **Шаг 1**: Нажмите 🔄 **Обновить** — список GGUF-моделей загружается через llama-swap
   - **Шаг 2**: Выберите модель, настройте параметры, нажмите **Сохранить**
