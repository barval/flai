...
| 🐛 **Парсер роутера: original_query для маркеров** | `_parse_router_response()` использует `original_query` для действий `image`, `video`, `camera`. Текст после маркера игнорируется. |
