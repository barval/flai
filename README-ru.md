...
| **Навыки — единый источник** | Все тексты возможностей вынесены в `prompts/{ru,en}/skills.txt`. `format_prompt()` автоматически подставляет `{skills_section}`. Ранее дублировались (и различались) в 4+ местах |
| **Ошибки фоновых задач изолированы** | Ошибки извлечения фактов и мержа логируются — никогда не попадают к пользователям через SSE. Фоновые задачи исключены из индикаторов ⚡/⏳ |
| **Стабильность счётчика очереди** | Фоновые задачи больше не сбивают пользовательский счётчик очереди в минус. `get_user_queue_counts()` возвращает `max(0, ...)` |
| **Rule-based извлечение SLM** | LLM-извлечение фактов заменено на pattern matching (только CPU, ~50-200мс). Семантическая дедупликация через `/similarity`. Нет блокировки GPU |
| **Rule-based мерж SLM** | LLM-мерж заменён на пайплайн: edit-distance + семантическое сходство + temporal decay. Автоархивация фактов старше 90 дней |
| **Отмена для всех типов задач** | Кнопка отмены для генерации/редактирования изображений и генерации видео (фоновая проверка + перезапуск контейнера). Стриминговые задачи через Redis |
| **Исправление auto-scroll чата** | Флаг `_isLoadingMessages` предотвращает гонку N async-колбэков скролла. `isNearBottom()` порог=200px. `overflow-anchor: none` |
| **Перевод ошибок llama-swap** | Ошибки llama-swap переводятся на язык пользователя через `_translate_llama_swap_error()` |
| **Исправление двойного ⚠️** | Сервер и клиент больше не добавляют "⚠️ " дважды — сервер владеет префиксом через `_build_error_response()` |
| **Очистка markdown для TTS** | `**жирный**`, `*курсив*`, `[ссылки]` и прочая разметка удаляется перед синтезом речи — никаких "звезда-звезда" в голосовых ответах. Обрабатываются фрагменты после разбивки по предложениям (`**НН.РУ**` → `НН.РУ`). Степени (`3**2=9`) сохраняются |
