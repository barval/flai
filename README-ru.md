...
|---------|------|-------|--------|
| Чат (Qwen3-4B-Instruct-2507) | ✅ полная скорость | ✅ полная скорость | ✅ полная скорость |
...
|--------|-----------|-------------|------|------|--------|-----------|------------|
| gemma-4-E2B-it-Q4_0 | Чат | Q4_0 | 3.0 ГБ | 2123 МБ | 1471 т/с | 168.1 т/с | Ультралёгкая edge-модель |
| **Qwen3-4B-Instruct-2507** | Чат | MXFP4 (MoE) | 2.0 ГБ | 3186 МБ | 3943 т/с | 127.7 т/с | **Текущая чат-модель** — самая быстрая обработка промпта |
...
> **Почему MTP не помогает на GPU с 128-bit шиной:** Multi-Token Prediction (MTP) предсказывает черновые токены маленькой головой, затем верифицирует их параллельно. На GPU с высокой пропускной способностью (256/512-bit шины) это даёт ускорение 1.4–2.2×. На 128-bit шине RTX 5060 Ti (448 GB/s) дополнительные чтения памяти draft-головы насыщают и без того ограниченную пропускную способность. Qwen3.5-4B-MTP оказывается **на 15% медленнее** Qwen3-4B без MTP; Qwen3.5-9B-MTP не показывает заметного ускорения по сравнению с обычным Q4_K_M того же размера.
...
├── Qwen3-4B-Instruct-2507-Q4_0.gguf          # Чат (не-Blackwell)
├── Qwen3-4B-Instruct-2507-MXFP4_MOE.gguf     # Чат (только Blackwell)
...
|----------|-----|-------------|-------------------|-----------|
| Длина контекста | 16384 | 16384 | 16384 | 512 |
| Температура | 0.7 | 0.7 | 0.7 | – |
| Top P | 0.9 | 0.9 | 0.9 | – |
...
|-----------|-------------|---------------------------|------------|
| **Чат** | Qwen3-4B-Instruct-2507 Q4_0 (~2.4 ГБ) | Qwen3-4B-Instruct-2507 MXFP4 (~2 ГБ) | Модель без рассуждений. Автоопределение: MXFP4 на Blackwell, Q4_0 на других GPU |
...
| **Мультимодальная** | Qwen3VL-8B Q4_K_M (~5 ГБ) | Qwen3VL-8B MXFP4 (~7.7 ГБ) | Требует поддиректорию с `mmproj-*.gguf` |
| **Эмбеддинг** | bge-m3 Q8_0 (~1.5 ГБ) | — | Единая модель для всех уровней |

> **Окна контекста:** Чат и модели рассуждений должны использовать одинаковую длину контекста (рекомендуется 16384). Мультимодальная требует ≥16384 для подсчёта токенов изображений.
...
- **Обновлена чат-модель** — Qwen3-4B-Instruct-2507 Q4_0 (~2.4 ГБ), дефолтный ctx 8192 → 16384. MXFP4-вариант автоопределяется на Blackwell GPU (RTX 5060+), Q4_0 на остальных архитектурах
...
|--------|-----------|---------|--------------|
| **Qwen3-4B-Instruct-2507-MXFP4_MOE.gguf** | Чат (Blackwell) | [Apache 2.0](https://huggingface.co/unsloth/Qwen3-4B-Instruct-2507-GGUF) | ~2.0 ГБ |
| **Qwen3-4B-Instruct-2507-Q4_0.gguf** | Чат (другие GPU) | [Apache 2.0](https://huggingface.co/unsloth/Qwen3-4B-Instruct-2507-GGUF) | ~2.4 ГБ |
