...
|--------------|------------|
| 🧠 **SLM в режиме демона** | SuperLocalMemory теперь работает как полноценный демон (`slm serve start`), постоянно держа MemoryEngine + модель эмбеддингов в памяти. Вместо per-request `subprocess --sync`. Задержка recall снижена с ~10с до ~300-800мс. HTTP-прокси (`slm_http.py`) перенаправляет запросы к демону. |
