...
| **Платформа в админ-API** | `/api/hardware` теперь сообщает определённую платформу (`nvidia` / `amd` / `intel` / `cpu`) наряду с именем GPU и VRAM. |
...
> **Текущий стек: CPU против GPU (три модели, с которыми поставляется ПЛИИ).**
