...
| 🧹 **Фильтр истории для роутера** | `_extract_text_content()` в `db.py` удаляет маркеры `[-VIDEO-]`, `[-IMAGE-]`, `[-REASONING-]`, `[-RAG-]`, `[-CAMERA-]`, `[-IMAGE-EDIT-]` и JSON `{"prefix": ..., "text": ...}` из истории диалога. Предотвращает копирование старых маркеров в новые ответы. |
