...
- 🧠 **Долговременная память** – кросс-сессионная память через SuperLocalMemory (SLM). Только CPU, rule-based извлечение и слияние фактов (без LLM). Семантическая дедупликация. Включить через `--profile with-slm`.
