...
| v8.4 (Новое) | Примечания |
|--------------|------------|
| 🔄 **Исправление стриминга изображений** | `queue.py:1392`: токены после маркера `[-IMAGE-EDIT-]` больше не отбрасываются при SSE-доставке; устранена ошибка пустого запроса редактирования |
| 🖥️ **GPU/CPU автоопределение для SD** | `sd_wrapper.py` определяет CUDA внутри контейнера через `nvidia-smi`; на GPU флаги `--offload-to-*` опускаются; `--cuda` не передаётся — sd-cli сам определяет |
| 🔁 **CUDA fallback** | Если `use_gpu=True`, но CUDA недоступна в контейнере SD — автоматический откат на CPU |
| 🌐 **Переводы ошибок SD восстановлены** | `_sd_error_translation_markers()` в `utils.py` для pybabel; 8 устаревших ключей `#~` в `.po` реактивированы с корректными ссылками |
| 🔧 **Исправление переключения сессий** | `chat-sessions.js`: вызов `loadMessages()` после удаления текущей сессии на сервере; клик по активной сессии перезагружает сообщения |
| 🗣️ **Глобальный перевод ошибок** | Все пользовательские сообщения обернуты в `_()` / `gettext()`; 14 новых ключей `.po`; правило добавлено в `AGENTS.md` — raw `str(e)` не возвращается пользователю |
| ⚡ **Исправление зависания ⚡ для аудио** | Исправлены гонки `clearSessionQueue` + `fetchQueueStatus` в HTTP-ответах с аудио без `request_id`; только одна сессия показывает ⚡ за раз |
| 🗑️ **`.gitignore *.pot`** | `messages.pot` исключён из версионного контроля |
