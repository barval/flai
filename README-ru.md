...
|--------------|------------|
| 🎨 Эмодзи и стиль служебных сообщений | Добавлен `🎨` к сообщениям генерации/редактирования изображений, пайпы вокруг времени ответа (`&#124; ⏱️ 5.8с &#124;`) |
| 🔧 Исправление SSE | Устранено 4 корневых причины: голосовые сообщения появляются без ручного обновления страницы |
| 🧰 Расширение CLI | `flask migrate-messages-format --add-emojis` — добавляет `🎨` к старым сообщениям в БД |
| 📱 Адаптация для планшетов | Добавлен медиа-запрос для диапазона 769-1199px — подвал больше не перекрывает строку ввода на планшетах |
| 📄 Извлечение PDF через pdftotext | PyPDF2 заменён на `pdftotext` (poppler-utils). Города и форматирование из сложных PDF (hh.ru и др.) сохраняются корректно. Резерв — pdfplumber. |
| ✂️ Chunk_text по символам | `chunk_text()` теперь делит по символам, а не по словам — единообразно с `chunk_text_recursive()`. Параметр `chunk_size` всегда в символах. |
| 🔢 Адаптивное ограничение `rag_top_k` | Автоматически ограничивается расчётным максимумом из контекста модели рассуждений; поле ввода обновляется после сохранения. |
| ⚡ flash_attn всегда на CUDA | Убран порог 24+ ГБ VRAM; flash_attn включён на любом CUDA GPU. Конфиг применяется через ResourceManager → `build_cmd()`. |
| 🔌 SSE-событие индексации документа | При завершении/сбое публикуется `document_indexed`; список документов обновляется в реальном времени. |
| 🔄 Повтор при ошибке эмбеддинга | До 3 повторов (5с/10с/15с) в `_get_batch_embeddings()` для временных сбоев при перезагрузке llama-swap. |
...
- **Система отладки** — `console.log`/`console.warn` заменены на `dlog`/`dwarn`, работающие только при `DEBUG_JS=true`
...
- **Чанкинг по символам** — единый параметр `chunk_size` в символах для стратегий fixed и recursive
...
- **Эмодзи и стиль служебных сообщений** — сообщения генерации/редактирования изображений снабжены `🎨`, время ответа обрамлено пайпами (`&#124; ⏱️ 5.8с &#124;`) для единообразия
