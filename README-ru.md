...
| **Улучшения голосового UX** | Прогрев Kokoro на холодный старт: первая русская фраза после развёртывания занимает ~2 с вместо ~56 с (`KOKORO_WARMUP_G2P`, `KOKORO_G2P_IDLE_TIMEOUT`, `KOKORO_TIMEOUT=120` — см. «Параметры настройки Kokoro» ниже). Фронтенд передаёт пол голоса явно в каждом запросе синтеза — переключение пола и немедленное воспроизведение больше не рискует старым голосом. |
