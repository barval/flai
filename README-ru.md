...
|--------------|------------|
| 🧠 **Интеграция SuperLocalMemory (SLM)** | Долговременная кросс-сессионная память через SuperLocalMemory (HTTP-прокси в отдельном контейнере, `--profile with-slm`). Zero-LLM поиск (метрика Fisher-Rao, только CPU). У каждого пользователя изолированная SQLite-БД (через `$HOME`, не `SLM_DATA_DIR`). Нет демона — вызовы через `--sync`. Предзагружается при сборке Docker-образа (с фоновым warmup при старте). Заменяет историю (~1255 токенов) на 3-5 релевантных фактов (~150 токенов). Автосохранение фактов после ответа ассистента. |
| ⚙️ **Фоновый импорт SLM при старте** | `app/slm_import.py` с checkpoint-таблицей `slm_import_progress`. При первом запуске (или обновлении со старой версии) автоматически импортирует все существующие сообщения в per-user SLM-БД. Инкрементально — только сообщения с последней точки сохранения. Запускается в daemon-потоке, не блокирует web-сервер. CLI: `flask import-history-to-slm [--force] [пользователь]`. |
| 🗑️ **Автоочистка SLM при удалении сеансов** | При удалении последнего сеанса или очистке истории `_cleanup_slm_if_empty()` в `db.py` удаляет SLM-БД пользователя. При полном удалении учётной записи удаляется весь каталог `/app/data/slm/{login}/`. |
| 📊 **Колонка SLM фактов в админ-панели** | `GET /admin/api/users` возвращает `slm_facts_count` на пользователя, читается напрямую из SQLite. Отображается в таблице пользователей. SLM-базы включены в полные бэкапы. |
| 🖥️ **GPU requirement + три уровня оборудования** | Режим CPU удалён. Требуется GPU (NVIDIA, от 8 GB VRAM). Три уровня: 8 GB (лёгкие модели), 12 GB (средние), 16+ GB (полные). Скрипты деплоя авто-определяют VRAM и скачивают подходящие модели. Обновлены лимиты ресурсов в docker-compose. |
| 🔄 **Улучшение управления VRAM** | `ensure_vram_for_llm()` в `resource_manager.py` проверяет VRAM перед LLM-запросами и выгружает LTX-Video пайплайн при необходимости. `unload_video_pipeline()` вызывается после SD и Video генерации. |
| 🐛 **Исправление парсинга ответа роутера** | `_parse_router_response()` берёт только первую строку после маркера. Предотвращает попадание скопированного текста шаблона и маркеров из истории в сгенерированный запрос. Исправляет баг «нарисован кот вместо яблока». |
