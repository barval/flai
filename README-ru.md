...
|-------------|------------|
| **Двухмодельная архитектура** | Отдельная «чат-модель» (Qwen3-4B) удалена. Мультимодальная модель (Qwen3VL-8B-Instruct) стала единственной чат/роутер/vision моделью — постоянно в VRAM (`ttl=0`, preload). После задач рассуждений мультимодальная модель перезагружается синхронно перед ответом. |
| **Упрощённая админ-панель** | Удалена отдельная карточка «Чат-модель» на странице моделей. Мультимодальная модель показывает объединённую роль chat/router/vision. |
| **Self-healing БД** | Устаревшие строки чат-модели из бэкапов v9.x автоматически удаляются при старте (`DELETE FROM model_configs WHERE module = 'chat'`). |
| **Синхронная перезагрузка мультимодальной** | После завершения рассуждений `_preload_multimodal_sync()` ждёт до 60 с пока мультимодальная модель не перейдёт в состояние running — нет задержки холодного старта на следующем запросе. |
| **Упрощённые скрипты деплоя** | `deploy.sh` / `deploy-ru.sh` больше не скачивают отдельную чат-модель Qwen3-4B. Минимальный конфиг llama-swap использует мультимодальную как default. |
...
|---------|------|-------|--------|
| Чат + Мультимодальная (Qwen3VL, всегда в VRAM) | ⚠️ Qwen3VL-4B (~2.5 ГБ) | ✅ Qwen3VL-8B (~5.5 ГБ) | ✅ Qwen3VL-8B (~5.5 ГБ) |
...
> **Управление VRAM:** Все LLM-модели (мультимодальная, рассуждения, эмбеддинг) разделяют VRAM через llama-swap — одновременно загружена только одна. SD и LTX-Video используют отдельные GPU-контексты с авто-выгрузкой LLM перед генерацией. Система динамически подстраивает `n_gpu_layers` под доступную VRAM.

### Бенчмарки моделей (RTX 5060 Ti 16 ГБ)

Реальные измерения на llama.cpp (загрузка моделей on-demand через llama-swap, Flash Attention, KV-кэш q4_0):

| Модель | Назначение | Квантизация | Файл | VRAM | Промпт | Генерация | Примечания |
|--------|-----------|-------------|------|------|--------|-----------|------------|
| gemma-4-E4B-it-Q4_0 | Рассуждения | Q4_0 | 4.8 ГБ | 3481 МБ | 1182 т/с | 99.8 т/с | Лучший баланс скорость/качество |
...
| gemma-4-12B-it-qat | Рассуждения | QAT Q4_K_XL | 6.3 ГБ | 7591 МБ | 1102 т/с | 48.7 т/с | Плотная 12B — на 62% медленнее |
| **Qwen3VL-8B-Instruct** | Мультимодальная | Q4_K_M | 4.7 ГБ | 5292 МБ | 2318 т/с | **73.1 т/с** | **Текущая мультимодальная модель** — самая быстрая |
| Qwen3VL-8B-Instruct-MXFP4 | Мультимодальная | MXFP4_MOE-Q6_K | 7.7 ГБ | 8222 МБ | 2099 т/с | 46.7 т/с | Визуальная модель — гибрид MXFP4 (устарела) |

> **Почему gpt-oss-20b — лучшая модель рассуждений:** Несмотря на «20B» параметров, gpt-oss-20b использует архитектуру Mixture-of-Experts (MoE) с 32 экспертами — за каждый токен активируется только ~3B параметров. Это даёт вычислительную стоимость 3B-модели с «знанием» 20B-модели. На RTX 5060 Ti генерация идёт на **118 т/с** против 63 т/с у плотной Qwen3.5-9B — **почти в 2 раза быстрее** при одинаковом использовании пропускной способности памяти.

> **Qwen3.6-35B-A3B как альтернатива для рассуждений:** Архитектура MoE (35B общих, ~3B активных) обеспечивает **106 т/с** — всего на 10% медленнее gpt-oss-20b. Сильный кандидат, если качество gpt-oss-20b недостаточно.
...
├── gemma-4-E4B-it-Q4_0.gguf                   # Рассуждения (8/12 ГБ)
├── gpt-oss-20b-Q4_K_M.gguf                    # Рассуждения (16+ ГБ, не-Blackwell)
├── gpt-oss-20b-mxfp4.gguf                     # Рассуждения (16+ ГБ, только Blackwell)
...
> **Архитектурно-зависимая загрузка:** Скрипты развёртывания автоматически определяют GPU Blackwell (RTX 5060+) и скачивают MXFP4-варианты для нативного FP4-ускорения. На других GPU (Ampere, Ada Lovelace) скачиваются стандартные Q4_0/Q4_K_M для оптимальной производительности.
...
|-----------|-------------|---------------------------|------------|
| **Чат/роутер/vision** | Qwen3VL-8B Q4_K_M (~5.5 ГБ) | Qwen3VL-8B MXFP4 (~7.7 ГБ) | Единая мультимодальная модель для всех трёх ролей; всегда в VRAM. На 8 ГБ используйте Qwen3VL-4B (~2.5 ГБ). Требует поддиректорию с `mmproj-*.gguf` |
| **Рассуждения (8/12 ГБ)** | Gemma 4 E4B Q4_0 (~4.8 ГБ) | — | Лучший баланс скорость/качество для средних GPU |
| **Рассуждения (16+ ГБ)** | gpt-oss-20b mxfp4/Q4_K_M (~12 ГБ) | Qwen3.6-35B-A3B Q2_K_XL (~12 ГБ) | MoE: ~3B активных параметров, ~118 т/с. Автоопределение: mxfp4 на Blackwell, Q4_K_M на других GPU |
...
- **Критические обновления зависимостей** — CVE-2026-25645 (`requests` 2.31→≥2.33) и CVE-2025-71176 (`pytest` 7.4→≥9.0); также `gunicorn`→26.1, `redis`→8.x, `qdrant-client`→1.19, `pytest-cov`→7.x
- **Переподключение RAG** — `RagModule` делает 3 попытки при старте и переподключается per-request; нет больше «RAG отключен» навсегда после перезапуска Qdrant
- **SLM recall в 2 раза быстрее** — два последовательных `slm.recall()` объединены в один; холодный старт ~39с→~25с, таймаут semantic 30с→15с
- **Защита генерации изображений от OOM** — sd-wrapper выбирает оптимальный offload-уровень по VRAM (веса модели + буфер VAE); нет двойного progress bar на 16 ГБ
- **Исправление зависания стриминга** — рендеринг токенов — throttle до 120 мс, sessionStorage — до 500 мс; устранён фриз браузера при длинных генерациях
- **Исправление роутера: поиск по документам** — несоответствие нумерации категорий; запросы «Кто такой X?» теперь попадают в `[-RAG-]` вместо `[-SEARCH-]`
- **Вызов инструментов: одинарные кавычки** — `ast.literal_eval()` fallback для маленьких LLM, выдающих Python-style dict
- **Исправление VRAM-регрессии** — `ensure_vram_for()` возвращает True если нужная модель уже загружена; сброшено stale `measured_vram_mb`
- **Миграция Qdrant v1.19.0** — client `search()`→`query_points()`; чистый старт с новым форматом сегментов; документ переиндексирован
- **Повторная попытка модели рассуждений** — если модель вернула только размышления (без финального ответа), задача повторяется один раз; устраняет случайные ошибки при холодных загрузках модели
...
- **Мультимодальная модель всегда горячая** — предзагрузка при старте через `hooks.on_startup`, TTL=0 (никогда не выгружается); перезагружается перед ответом после каждой задачи рассуждений, нет холодных стартов
...
- **Корректность TTL** — мультимодальная=0 (никогда не выгружать), остальные модели=1 (выгрузка через 1с простоя). Предыдущие значения были инвертированы
...
- **Управление GGUF-моделями** через админ-панель — настройка для каждого модуля (мультимодальность, рассуждения, эмбеддинг) из веб-интерфейса
...
- **SuperLocalMemory (SLM)** — долговременная кросс-сессионная память, daemon-режим, per-user изоляция SQLite, задержка recall ~1 мс, rule-based извлечение и слияние фактов (без LLM), семантическая дедупликация, temporal decay, автоматическая очистка осиротевших memories
...
- **Адаптивная деградация моделей** — итеративное снижение `n_gpu_layers` при OOM, отдельные circuit breaker по типу модели, retry reasoning при 500/502 с деградацией, flash-attn отключается при частичном offloading
...
- **Fast worker GPU lock** — мультимодальность, эмбеддинг, RAG-поиск тоже берут `_gpu_lock`, исключая параллельные GPU-задачи
...
- **TTL-оптимизация VRAM** — мультимодальная модель всегда горячая (TTL=0), остальные модели выгружаются через 1с после ответа (TTL=1с)
...
- **Дефолтная мультимодальная модель** — Qwen3VL-8B-Instruct Q4_K_M (~5.5 ГБ) выполняет роли чат/роутер/vision, всегда в VRAM; дефолтный ctx 8192 → 16384; Qwen3VL-4B (~2.5 ГБ) для 8 ГБ GPU
- **Модели рассуждений** — 8/12 ГБ: Gemma 4 E4B Q4_0 (~4.8 ГБ), 16 ГБ+: gpt-oss-20b mxfp4 на Blackwell / Q4_K_M на других GPU (~12 ГБ)
...
| **gpt-oss-20b-mxfp4.gguf** | Рассуждения (16 ГБ+, Blackwell) | [OpenAI License](https://huggingface.co/unsloth/gpt-oss-20b-GGUF) | ~12 ГБ |
| **gpt-oss-20b-Q4_K_M.gguf** | Рассуждения (16 ГБ+, другие GPU) | [OpenAI License](https://huggingface.co/unsloth/gpt-oss-20b-GGUF) | ~12 ГБ |
...
|--------------|-----------------|
| Минимальный (Qwen3VL-4B + Gemma 4 E4B + bge-m3, уровень 8 ГБ) | ~10 ГБ |
| Полный LLM-стек (Qwen3VL-8B + gpt-oss-20b + bge-m3) | ~20 ГБ |
