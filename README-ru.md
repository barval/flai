...
| **Гибкость CUDA-драйверов (в работе)** | Запуск FLAI на любом драйвере хоста начиная с CUDA 12.2: скрипты развёртывания сами определяют драйвер, адаптируют выбор образов и предупреждают, когда отдельным функциям нужен более свежий драйвер. |
...
| CUDA-драйвер хоста | Используемые образы |
|--------------------|---------------------|
...
| **12.2 – 12.3 (минимум)** | CUDA 12.2.2 (Ubuntu 22.04) |

> ⚠️ **Исключение для LTX-Video:** образ LTX-Video построен на `pytorch:2.5.0-cuda12.4-runtime` и требует **драйвер ≥ 550.54.14 (CUDA 12.4)**. На хостах с CUDA 12.2–12.3 основной стек (чат, reasoning, RAG, TTS, Whisper, генерация изображений) работает полностью, но профиль `with-video` запуститься не сможет — разворачивайтесь без него либо обновите драйвер NVIDIA (достаточно обновить только драйвер, переустановка toolkit не требуется).
