...
|-------------|------------|
| **v11.5 — рассуждения на свежих данных из интернета** | Новая категория роутера `[-REASONING-WEB-]` покрывает сложные запросы, которым нужны ещё и актуальные данные из интернета (анализ, сравнения, обзоры по свежим фактам). Такой запрос теперь сначала собирает результаты SearXNG на fast-воркере, а затем рассуждает над ними, сохраняя всю историю сессии — раньше такие продолжения попадали в `[-REASONING-]`, который работает без интернета, и модель отвечала по устаревшим весам. При недоступном или пустом поиске происходит тихий откат к обычным рассуждениям вместо ошибки. |
| **Конвертация по текущему курсу — через веб-поиск** | Запрос на конвертацию валюты/единиц («переведи £3,293.31 по текущему курсу в рубли») всегда классифицируется как веб-поиск, а не как офлайн-рассуждение, и поисковики опрашиваются именно о курсе — запрос с дробной суммой иначе возвращает только виджет-конвертер без ссылок. Дробная сумма срезается для повторной попытки поиска, а модель рассуждений по-прежнему получает исходный вопрос пользователя (с суммой) и может посчитать. |
| **Рассуждения больше не падают из-за watchdog-гонки** | Фоновый health-check watchdog'а мог возродить только что выгруженную мультимодальную модель прямо во время ожидания VRAM для модели рассуждений — примерно каждый четвёртый такой запрос падал с «проверка памяти GPU не удалась». Теперь GPU-транзакции (генерация изображений/видео, смена моделей) полностью приостанавливают watchdog, а ожидание VRAM повторно выгружает всё возродившееся вместо таймаута. |
| **v11.4 — надёжный стрим и информативный интерфейс** | Всё, что вышло после релиза v11.3: устойчивость стрима (детектор зацикливания, лимит размышлений), умный поиск (нормализация дат) и полностью информативная строка прогресса в чате. |
| **Информативные стадии прогресса** | Чат показывает, что реально происходит между запросом и ответом: «🔍 Анализирую запрос... → 📚/🌐 Ищу... (N с) → Найдено X результатов... → 🧠 Загружаю модель рассуждений... → 🤔 Обдумываю ответ... (N с) → ⚡ Генерация». Стадия размышления приходит по новому бэкенд-колбэку `status_callback`, срабатывающему в момент, когда модель загружена и генерация началась (раньше весь тихий период размышления выглядел как «Загружаю модель рассуждений...»). Каждый вызов любого chat-tool получает свою стадию — 🕐 `get_current_time` (Получаю время...), 🔢 `calculator` (Вычисляю...), 📅 `time_calc` (Рассчитываю дату...), 🌐 `web_search` (Ищу в интернете...), 📚 `rag_search` (Ищу в документах...), 📹 `camera_snapshot` (Делаю снимок...) — поэтому погодный или новостной запрос, отвеченный через вызов инструмента, всегда показывает статус поиска. Каждая фаза тикает затраченные секунды. |
| **Детектор зацикливания в стриме** | Стриминговые ответы рассуждений проходят через loop guard (`_LoopGuard`, удержание 1500 символов): обнаруженные текстовые повторы обрезаются на лету, не заливая чат; хвосты на конце стрима флашатся в фиксированном порядке, поэтому короткие ответы никогда не теряются и не переставляются. Серверный safety-net ловит повторы, пропущенные в стриме. |
| **Лимит размышлений enforced** | llama-server build 10603 игнорирует per-request поле `reasoning_budget`, поэтому команда reasoning-модели теперь ограничивает мышление через CLI-флаг `--reasoning-budget max(1024, ctx*0.4)` — контекст больше не сгорает целиком на размышлениях без ответа. |
| **Нормализация дат в поиске** | Относительные даты в поисковых запросах («вчера», «сегодня», английские аналоги) разрешаются в абсолютные даты в таймзоне пользователя перед отправкой в SearXNG — движки возвращают датированные статьи, а не общие лендинги новостных разделов. |
| **Гибкость CUDA-драйверов** | Запуск полного GPU-стека (чат, reasoning, генерация изображений и LTX-Video) на любом драйвере хоста начиная с CUDA 12.2: `deploy-ru.sh` сам определяет драйвер, адаптирует выбор образов и отключает требования NVIDIA к образам через `NVIDIA_DISABLE_REQUIRE=1` там, где позволяет minor-version compatibility (проверено пользователями на RTX 3090 + CUDA 12.2). GPU-режим требует CUDA ≥ 12.2; только отсутствие NVIDIA GPU или CUDA < 12.2 переводит в режим CPU. |
...
|----------|-------------------|-------------|-----------|
| Длина контекста | 32768 (авто-подбор: 16384 на 8 ГБ, 8192 CPU) | 24576 (авто-подбор: 16384 на 8 ГБ, 8192 CPU) | 512 |
| Температура | 0.7 | 0.7 | – |
| Top P | 0.9 | 0.9 | – |
...
| `KOKORO_TIMEOUT` | `120` | Клиентский таймаут веб-приложения на один запрос синтеза. Покрывает холодную загрузку RUAccent (~10–20 с) плюс перезагрузку модели под нагрузкой хоста; холодный старт раньше подходил вплотную к старому лимиту 60 с и вызывал клиентские таймауты. |
