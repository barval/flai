...
- **Изоляция SLM через прямой SQLite** — recall с profile читает напрямую из per-user SQLite (`atomic_facts`). Общая БД демона не используется для пользовательских запросов (~1мс). Remember сохраняет в обе БД (демон + per-user, асинхронно).
