...
ПЛИИ работает на **NVIDIA GPU с поддержкой CUDA** (рекомендуется для максимальной скорости). Начиная с **v11.0**, также доступен **режим только CPU** — LLM инференс, генерация изображений и видео работают без GPU, только значительно медленнее.
...
| **ОЗУ** | 16 ГБ | 16 ГБ | 16 ГБ | 32+ ГБ |
| **ЦПУ** | 4+ ядра | 4+ ядра | 6+ ядер | 8+ ядер |
...
|---------|------|-------|--------|-----------|
| Чат + Мультимодальная (Qwen3VL, всегда в VRAM) | ⚠️ Qwen3VL-4B (~2.5 ГБ) | ✅ Qwen3VL-8B (~5.5 ГБ) | ✅ Qwen3VL-8B (~5.5 ГБ) | ⚠️ медленнее |
| Рассуждения | ✅ Gemma 4 E4B (~4.8 ГБ) | ✅ Gemma 4 E4B (~4.8 ГБ) | ✅ gpt-oss-20b (~12 ГБ) | ⚠️ медленнее |
...
| Генерация видео (LTX-Video) | ⚠️ 512×512×120 кадров | ✅ 768×512×240 кадров | ✅ 768×512×240 кадров | ⚠️ 256×384×49 кадров |
...
- Linux-сервер (**NVIDIA GPU** с CUDA рекомендуется, но не обязателен начиная с v11.0)
- Для режима GPU: установлены **драйверы NVIDIA** и **NVIDIA Container Toolkit**
- Docker Engine ≥ 20.10
- Docker Compose ≥ 2.0
