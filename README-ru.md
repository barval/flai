...
Всё выполняется **локально** одной GPU-задачей: модель рассуждений держится загруженной на всю длительность анализа, прогресс транслируется вживую («Читаю документы...», «Шаг анализа N...»), а ответ приходит с схлопывающейся сводкой **«🔬 Глубокий анализ (N шагов)»**.
...
|-------------|------------|
| **Режим «Глубокий анализ» (RLM)** | Выделенный переключатель «🔬 Глубокий анализ» ведёт ваш вопрос и выбранные документы через цикл модели-исследователя до 18 шагов. Бюджет по железу следует лестнице: 24 ГБ+ → 18, 16 ГБ → 12, 12 ГБ → 10, 8 ГБ → 8, CPU/<8 ГБ → 6. Модель использует изолированную песочницу `python`, под-вызов `llm()` и до 2 живых `web_fetch`-обращений, когда нужны свежие факты (5 — жёсткий потолок). Одна GPU-задача удерживает модель рассуждений на всё время анализа; прогресс транслируется по шагам, а ответ сопровождается схлопывающейся сводкой «🔬 Глубокий анализ (N шагов)». Как пользоваться — в разделе [Режим «Глубокий анализ»](#-режим-глубокий-анализ) ниже. |
| **Защита от OOM и адаптивный бюджет шагов** | Суммарный размер корпуса ограничен (по умолчанию 50 млн символов, `RLM_MAX_CORPUS_CHARS`): слишком большие наборы документов отклоняются до любой GPU-работы. Наблюдения каждого шага сжимаются под окно контекста, поэтому машины с меньшими ресурсами завершают свой аппаратный бюджет шагов без ошибки «Request too long». |
| **Выбор документов кликом, изображения попадают в корпус** | Документы для анализа выбираются кликом в списке документов (зелёная рамка + ✓, живой счётчик у переключателя). Приложенное изображение мультимодальная модель подробно описывает, и это описание становится ещё одним «документом» анализа — например, «сопоставь фото гарантийного талона с пунктом 4 договора». Если анализ не может стартовать (нет документов и изображения, либо картинка без вопроса) — переключатель снимается автоматически, и запрос уходит в обычный поток. |
| **OCR сканированных PDF и изображения документов** | PDF без извлекаемого текста разбираются постранично: Poppler создаёт изображения страниц, мультимодальная модель распознаёт текст. Загруженные изображения документов описываются моделью и индексируются как текст для поиска. |
| **Токены в каждом ответе** | В заголовке сообщения показываются реальные количества токенов запроса и ответа (`🔢 (▲выход ▼вход)`), накопленные по всем вызовам моделей в запросе. |
| **Переводы гарантированы при каждом клоне** | Скомпилированные файлы `.mo` зафиксированы в репозитории, а скрипты деплоя компилируют переводы до первого запуска — свежий клон/развёртывание всегда получает полностью локализованный интерфейс без лишних шагов. |
