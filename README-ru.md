...
| **Переподключение RAG** | `RagModule` делает 3 попытки при старте и переподключается per-request; нет больше «RAG отключен» навсегда после перезапуска контейнеров |
...
| **Защита генерации изображений от OOM** | sd-wrapper выбирает оптимальный offload-уровень по VRAM (веса модели + буфер декодирования VAE); нет двойного progress bar на 16 ГБ |
...
| **Вызов инструментов: одинарные кавычки** | `ast.literal_eval()` fallback для маленьких LLM, выдающих Python-style dict с одинарными кавычками |
| **Повторная попытка модели рассуждений** | Если модель рассуждений вернула только блок размышлений, задача повторяется один раз. При повторе модель уже загружена — вторая попытка быстрая. |
