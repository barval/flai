...
| 🌐 `web_fetch` | ищет в интернете (SearXNG) до 5 дополнительных запросов, когда для ответа нужны свежие факты |
...
**Как пользоваться:**
1. Загрузите нужные файлы в **Документы** (PDF/DOC/DOCX/TXT).
...
|-------------|------------|
| **Режим «Глубокий анализ» (RLM)** | Выделенный переключатель «🔬 Глубокий анализ» ведёт ваш вопрос и выбранные документы через цикл «модель-исследователь» (до 12 шагов), который программно прорабатывает материал: песочница-`python` (разбить/подсчитать/разобрать/посчитать), вызов под-модели `llm()` и до 5 живых `web_fetch`-обращений, когда нужны свежие факты. Вся задача — одна GPU-задача, удерживающая модель рассуждений на всё время анализа; прогресс транслируется по шагам, а ответ сопровождается схлопывающейся сводкой «🧩 Глубокий анализ (N шагов)». Как пользоваться — в разделе [Режим «Глубокий анализ»](#-режим-глубокий-анализ) ниже. |
