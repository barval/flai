...
| 🧹 **Очистка CUDA после видео** | `torch.cuda.synchronize()`, `torch.cuda.empty_cache()`, `gc.collect()` в ltx_wrapper.py после каждой генерации, плюс `_pipeline = None` для ленивого пересоздания. Повторная выгрузка LLM через `unload_llamacpp_model()` в `finally` блоке video.py. `cuDevicePrimaryCtxReset(0)` удалён — вызывал SIGSEGV при перезагрузке пайплайна. |
...
| ⚙️ **Оптимизация VRAM для reasoning** | Снижены `--n-gpu-layers` с 24 до 16 и `--ctx-size` с 32768 до 16384 для модели рассуждений. Предотвращает OOM на GPU 16 GB, особенно после видеогенерации. |
| 📏 **Отображение размера файлов** | `file_size` читается с диска для сообщений с `file_path` и отображается в заголовках чата (например, `video.mp4, 2.1МБ`). Работает для всех типов файлов. |
| 🐛 **Исправление fileSize в chat-init.js** | `window.displayMessage` в `chat-init.js` переопределял функцию с 16 параметрами, теряя `fileSize`. Добавлен 17-й параметр и передача в оригинальную функцию. |
| 🔧 **Исправление SSE-обработчиков** | Три обработчика (`onResultCompleted`, `handleStreamComplete`, `onTaskResult`) не передавали `file_size` в `displayMessage`. Добавлен параметр `msg.file_size` / `result.file_size`. |
| ✂️ **Упрощение имён видеофайлов** | Имя изменено с `video_{timestamp}_{seed}_{W}x{H}x{F}.mp4` на `{timestamp}.mp4`. Seed и разрешение остаются в метаданных. |
...
ПЛИИ может работать на серверах без GPU благодаря автоматическому определению в скрипте развёртывания. Если видеокарта NVIDIA не обнаружена, скрипт использует образы для CPU. Производительность будет заметно ниже, но все функции кроме мультимодального анализа (зрение) и генерации видео останутся доступны.
