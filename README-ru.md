...
|-------------|------------|
| **Улучшение поиска в интернете** | Страницы с короткими сниппетами (<300 символов) загружаются параллельно (ThreadPoolExecutor, макс 3 потока) для извлечения текста через trafilatura. Инструкция смягчена с «ИСПОЛЬЗУЙ ТОЛЬКО ЭТИ ДАННЫЕ» на «используй как основной источник», позволяя модели дополнять ответ своими знаниями при нехватке данных. Контент урезается до 2 000 символов на результат и динамически ограничивается суммарно (~30% эффективного бюджета контекста). |
| **Упрощение Docker volumes** | 35 отдельных файловых mount'ов заменены на 3 директории (`./app:/app/app`, `./modules:/app/modules`, `./prompts:/app/prompts`) для hot-reload. `PYTHONDONTWRITEBYTECODE=1` предотвращает `__pycache__` на хосте. |
