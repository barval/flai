...
| 🧹 **Очистка CUDA после видео** | `torch.cuda.synchronize()`, `torch.cuda.empty_cache()`, `gc.collect()`, плюс `cuDevicePrimaryCtxReset(0)` в ltx_wrapper.py после каждой генерации. Повторная выгрузка LLM через `unload_llamacpp_model()` в `finally` блоке video.py. |
