...
| Модель | Назначение | Квантизация | Размер | VRAM | Промпт | Генерация | Примечания |
|--------|-----------|-------------|--------|------|--------|-----------|------------|
| **Qwen3-4B-Instruct-2507** | Чат | MXFP4 (MoE) | 2.0 ГБ | 3668 МБ | 1307 т/с | **127.4 т/с** | Текущая чат-модель |
| Qwen3.5-4B-Instruct-MTP | Чат | MXFP4 + MTP | 2.5 ГБ | 4042 МБ | 664 т/с | 108.2 т/с | MTP добавляет накладные расходы |
| **gpt-oss-20b** | Рассуждения | MXFP4 (MoE) | 11.5 ГБ | 12255 МБ | 1100 т/с | **120.5 т/с** | Текущая модель рассуждений |
| Qwen3.5-9B-MTP-Q4_K_M | Рассуждения | Q4_K_M + MTP | 5.5 ГБ | 6717 МБ | 431 т/с | 66.1 т/с | На 45% медленнее |
| Qwen3.5-9B-Q8_0 | Рассуждения | Q8_0 | 8.9 ГБ | 9719 МБ | 472 т/с | 42.8 т/с | На 65% медленнее |
| gemma-4-12B-it-qat | Рассуждения | QAT Q4_K_XL | 6.3 ГБ | 8090 МБ | 520 т/с | 49.2 т/с | На 59% медленнее |

> **Почему gpt-oss-20b — лучшая модель рассуждений:** Несмотря на «20B» параметров, gpt-oss-20b использует архитектуру Mixture-of-Experts (MoE) с 32 экспертами — за каждый токен активируется только ~3B параметров. Это даёт вычислительную стоимость 3B-модели с «знанием» 20B-модели. На RTX 5060 Ti генерация идёт на **120 т/с** против 66 т/с у плотной Qwen3.5-9B — **почти в 2 раза быстрее** при одинаковом использовании пропускной способности памяти.
