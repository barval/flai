...
| 🧪 **34 новых теста** | `tests/test_classify_model_fit.py` (11), `tests/test_dry_load.py` (10), `tests/test_health_monitor.py` (12). Все проходят. |
...
- **5-уровневая защита моделей в админ-панели** — 3-уровневая VRAM/RAM классификация (🟢 good / 🟡 cpu_offload / 🔴 impossible / ⚠ unknown), серверная валидация блокирует плохие сохранения HTTP 400, фоновый dry-load + авто-откат, watchdog с 5-минутным скользящим окном. `app/tasks/dry_load.py`, `app/tasks/health_monitor.py`, `_classify_model_fit()` в `app/routes/admin.py`.
- **Динамическая оценка VRAM по метаданным GGUF** — `_estimate_model_vram()` использует `file_size_mb × (ngl/block_count) + ctx_size × kv_factor + overhead`. Новая таблица `model_vram_estimates` хранит оценку и замеры. В админ-панели — цветные полоски процентов.
- **Отдельные circuit breaker по типу модели** — `LlamaSwapBackend._get_circuit_breaker(model_type)` изолирует сбои chat / reasoning / multimodal / embedding. Адаптивная деградация ngl при каждом сбое.
- **Генерация RAG на slow worker** — `_process_rag_task*` заменил `rag.generate_answer()` на `rag.search()` + `_requeue_reasoning_task(rag_context=...)`. Рассуждающая модель теперь видит сырые чанки Qdrant даже при сбое RAG LLM. Промпт RAG исправлен: использовать ТОЛЬКО контекст.
- **Поддержка нескольких вкладок** — клиент отправляет `session_id` в теле запроса (UUID v4). Сервер проверяет владельца. Устранены гонки cookie.
- **Префикс сообщений об ошибках** — `_build_error_response()` автоматически добавляет `⚠️ `. Хелпер `_is_llm_error_string()` направляет ошибки `call_llamacpp()` через него. Применено в reasoning, text-task, RAG.
- **Усиление VRAM видео** — try/finally в обоих обработчиках видео, `_unload_video_pipeline()` + `_unload_llamacpp_models()` выполняются всегда, сброс CUDA-кэша после генерации, таймаут 30 → 60 с, буфер +500 → +3000 МБ, эндпоинт `/v1/vram_info` в ltx-wrapper для динамических замеров.
- **Исправление системы переводов** — убраны сломанные `.mo` маунты из `docker-compose.gpu.yml`. Docker компилирует переводы при сборке.
- **Очистка Docker compose** — удалены `docker-compose.cpu.yml` (CPU-only не поддерживается), `services/llamacpp/generate_presets.py` (устарел), `services/sd_cpp/Dockerfile.sd_cpp-cpu`.
- **Обновлена дефолтная чат-модель** — `Qwen3-4B-Instruct-2507-MXFP4_MOE.gguf` (~2 ГБ, быстрее), дефолтный ctx 8192 → 16384.
- **Deploy-скрипты: определение уровня VRAM** — авто-выбор рассуждающей модели по `nvidia-smi`: 16+ ГБ → gpt-oss-20b, 12 ГБ → Qwen3-8B-Thinking, 8 ГБ → Qwen3-4B-Thinking.
- **34 новых теста** — `tests/test_classify_model_fit.py` (11), `tests/test_dry_load.py` (10), `tests/test_health_monitor.py` (12).
- **Reasoning 502 → повтор с деградацией** — `max_retries = 1` для reasoning и chat. Первый сбой → понижение ngl; второй сбой → пользовательская ошибка.
- **Fast worker берёт GPU lock** — чат, эмбеддинг, RAG-поиск теперь тоже берут `_gpu_lock` (раньше только slow worker). Исключает параллельные GPU-задачи.
