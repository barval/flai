...
|-------------|------------|
| **Режим «Глубокий анализ» (RLM)** | Выделенный переключатель «🔬 Глубокий анализ» ведёт ваш вопрос и выбранные документы через цикл «модель-исследователь» (до 12 шагов — бюджет автоматически подстраивается под окно контекста), который программно прорабатывает материал: песочница-`python` (разбить/подсчитать/разобрать/посчитать), вызов под-модели `llm()` и до 2 живых `web_fetch`-обращений (5 — жёсткий потолок), когда нужны свежие факты. Вся задача — одна GPU-задача, удерживающая модель рассуждений на всё время анализа; прогресс транслируется по шагам, а ответ сопровождается схлопывающейся сводкой «🔬 Глубокий анализ (N шагов)». Как пользоваться — в разделе [Режим «Глубокий анализ»](#-режим-глубокий-анализ) ниже. |
...
| **Защита от OOM и адаптивный бюджет шагов** | Суммарный размер корпуса ограничен (по умолчанию 50 млн символов, `RLM_MAX_CORPUS_CHARS`): слишком большие наборы документов отклоняются с понятной ошибкой до какой-либо GPU-работы. Бюджет шагов вычисляется из окна контекста модели рассуждений, чтобы траектория всегда в него уместилась — никаких «Request too long» посреди анализа на слабых машинах. |
...
MAX_DOCUMENT_SIZE_MB=5          # Макс. размер загружаемого документа
...
RLM_MAX_STEPS=12                # Потолок цикла (бюджет автоматически подстраивается под окно контекста)
RLM_TASK_TIMEOUT=900            # Общий дедлайн анализа по времени (секунды, 0 отключает)
...
### ✔️ Выполнено в v11.5
- **Рассуждения на свежих данных из интернета** — новая категория роутера `[-REASONING-WEB-]` позволяет сложному продолжению диалога, которому нужны актуальные данные, сначала выполнить поиск (SearXNG, на CPU-стороне), а затем рассуждать над результатами с сохранением истории сессии; при сбое поиска — тихий откат к обычным рассуждениям
- **Конвертация по текущему курсу** — запросы на конвертацию валюты/единиц всегда классифицируются как веб-поиск (никогда не офлайн-рассуждение), и дробная сумма больше не приводит к нулевым результатам поиска
- **Надёжная загрузка модели рассуждений** — watchdog/queue-гонка за VRAM (health-check возрождает модель посреди выгрузки) больше не валит задачи рассуждений с «проверка памяти GPU не удалась»

### ✔️ Выполнено в v11.3
- **Качество контекста и поиска** — умный бюджет контекста для истории сообщений, более качественный RAG и веб-поиск, более плотная долговременная память SLM
