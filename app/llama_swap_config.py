...
DEFAULT_TTL = {
    ...
    "multimodal": 0,  # never unload — always resident (router + chat + vision)
}
...
class LlamaSwapConfigGenerator:
    ...
    def _get_original_ngl(self, module: str) -> int | None:
        ...
        try:
            ...
            if ngl == -1:
                ...
                defaults = {"embedding": 12, "reasoning": 48, "multimodal": 32}
    ...
    def build_merge_cpu_entry(self) -> dict[str, Any] | None:
        """Build CPU-only model entry for fact_merge (same model as multimodal, runs on CPU)."""
        config = get_model_config("multimodal")
        if not config:
            self.logger.warning("No multimodal config for merge_cpu")
            return None

        model_name = config.get("model_name")
        model_path = self.get_model_path("multimodal", model_name)
        if not model_path:
    ...
    def build_cmd(
        ...
        try:
            ...
            if gguf_cache.get(model_key, {}).get("supports_mtp") and not is_cpu:
    ...
    def generate_yaml(self, ngl_overrides: dict[str, int] | None = None, include_preload: bool = True) -> str:
        ...
        Args:
            ...
            include_preload: Whether to include on_startup preload hook for multimodal model.
        """
        ngl_overrides = ngl_overrides or {}
        lines = [
            "# Generated by FLAI v11.0",
            "",
            "logLevel: info",
        ...
        groups = {
            "llm_fast": {
                "swap": True,
                "models": ["embedding", "reasoning", "multimodal"],
            },
            "cpu_only": {
                "swap": False,
                "models": ["merge_cpu"],
        ...
        # Preload multimodal model at startup via llama-swap hooks
        ...
        for module in ["embedding", "reasoning", "multimodal"]:
