...
class LlamaCppClient:
    ...
    def rerank(
        ...
        """
        Rerank documents by relevance using llama.cpp /v1/rerank endpoint.

        Args:
            query: The search query
            documents: List of document texts to rerank
            model_type: Module type (should be 'reranker')
        ...
        config = get_model_config(model_type)
        if not config:
            self.logger.warning(f"No model config for {model_type}, reranking disabled")
            return None
        ...
        self.logger.info(
            f"Sending rerank request to {service_url}, model={model}, "
            f"docs={len(documents)}, timeout={timeout}s"
        )
        try:
            response = requests.post(
                f"{service_url}/v1/rerank",
                json=payload,
                timeout=timeout
            )
            if response.status_code == 200:
                ...
                if not results:
                    self.logger.warning("Reranker returned no results")
                    return []

                # Sort by relevance_score descending (should already be sorted)
                results.sort(key=lambda x: x.get('relevance_score', 0), reverse=True)
                self.logger.info(
                    f"Rerank successful: {len(results)} results, "
                    f"top score={results[0].get('relevance_score', 0):.4f}"
                )
                return results
            else:
                self.logger.error(
                    f"Rerank error: {response.status_code} - {response.text}"
                )
                return None
        except requests.exceptions.Timeout:
            self.logger.error(f"Timeout calling rerank endpoint at {service_url}")
            return None
        except requests.exceptions.ConnectionError:
            self.logger.error(f"Connection error to llama-server at {service_url}")
            return None
