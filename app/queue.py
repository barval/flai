...
class RedisRequestQueue:
    ...
    def start_worker(self):
        ...
        thread.start()
        current_app.logger.info("RedisRequestQueue: воркер запущен")

    def _worker_loop(self):
        current_app.logger.info("RedisRequestQueue: запуск цикла обработки")
        while True:
            try:
                ...
                if queue_time > 300:  # 5 минут
                    current_app.logger.warning(f"Задача {task['id']} слишком долго ждала в очереди ({queue_time:.1f}с). Отмена.")
                    ...
                    continue
                current_app.logger.info(f"RedisRequestQueue: получена задача {task['id']} из очереди для сеанса {task['session_id']}, ожидание в очереди: {queue_time:.1f}с")
                ...
                try:
                    ...
                    current_app.logger.info(f"RedisRequestQueue: задача {task['id']} выполнена успешно для сеанса {task['session_id']}")
                except Exception as e:
                    current_app.logger.error(f"RedisRequestQueue: ошибка обработки задачи {task['id']}: {str(e)}")
                    self.redis.hset(self.results_key, task['id'], pickle.dumps({
                        'status': 'error',
                        'error': str(e),
                        'result': {'session_id': task['session_id']},
                ...
                current_app.logger.error(f"RedisRequestQueue: ошибка в worker loop: {str(e)}")
                time.sleep(1)
    ...
    def add_request(self, user_id, session_id, request_data, user_class):
        ...
        current_app.logger.info(f"RedisRequestQueue.add_request: добавление задачи {request_id} для сеанса {session_id} с временем {timestamp}")
        ...
        current_app.logger.info(f"RedisRequestQueue.add_request: задача добавлена, позиция={queue_length}")
    ...
    def _get_session_title(self, session_id):
        ...
        try:
            ...
            current_app.logger.error(f"Ошибка получения заголовка сессии: {str(e)}")
            return "Неизвестный сеанс"

    def _process_request(self, task):
        current_app.logger.info(f"RedisRequestQueue._process_request: обработка задачи {task['id']} для сеанса {task['session_id']}")
        user_id = task['user_id']
        session_id = task['session_id']
        request_data = task['data']
        processing_start_time = time.time()
        current_time_str = current_app.modules['base'].get_current_time_in_timezone()  # предполагаем, что BaseModule имеет такой метод
        ...
        if request_type == 'text':
            ...
            router_result = current_app.modules['base'].process_message(message_text, current_time_str)
            ...
            final_response = ""
            model_used = current_app.config['LLM_CHAT_MODEL']
            is_error = False
            process_time = 0

            if action_type == 'image':
                if 'image' in current_app.modules and current_app.modules['image'].available:
                    mm_start_time = time.time()
                    prompt_data, error = current_app.modules['multimodal'].generate_image_params(query)
                    ...
                    if error:
                        ...
                        image_result = current_app.modules['image']._call_automatic1111(prompt_data)
                        gen_time = round(time.time() - gen_start_time, 1)
                        if image_result['success']:
                            completion_time_for_db = get_current_time_in_timezone_for_db()
                            image_result['mm_time'] = mm_time
                            image_result['gen_time'] = gen_time
                            image_result['mm_model'] = current_app.config['LLM_MULTIMODAL_MODEL']
                            image_result['gen_model'] = current_app.config['AUTOMATIC1111_MODEL']
                            ...
                            save_message(
                                ...
                                image_result['file_name'], current_app.config['AUTOMATIC1111_MODEL'],
                                response_time={'mm_time': mm_time, 'gen_time': gen_time},
                                mm_time=str(mm_time), gen_time=str(gen_time),
                                mm_model=current_app.config['LLM_MULTIMODAL_MODEL'],
                                gen_model=current_app.config['AUTOMATIC1111_MODEL']
                            )
                            return {
                                'response': message_text,
                                'session_id': session_id,
                                'model_used': current_app.config['AUTOMATIC1111_MODEL'],
                                'assistant_timestamp': completion_time_for_db,
                ...
                if 'cam' in current_app.modules and current_app.modules['cam'].available:
                    ...
                    camera_result = current_app.modules['cam'].get_snapshot(query)
                    ...
                    if camera_result['success']:
                        ...
                        if message_text and 'multimodal' in current_app.modules and current_app.modules['multimodal'].available:
                            mm_start_time = time.time()
                            bot_reply, error = current_app.modules['multimodal'].process_image_with_text(
                            ...
                            save_message(
                                ...
                                model_name=current_app.config['LLM_MULTIMODAL_MODEL'],
                                response_time=str(mm_time)
                            )
                            second_message = {
                                'response': bot_reply,
                                'session_id': session_id,
                                'model_used': current_app.config['LLM_MULTIMODAL_MODEL'],
                ...
                if router_result.get('needs_reasoning'):
                    ...
                    final_response = current_app.modules['base'].process_reasoning(query, current_time_str)
                    process_time = round(time.time() - reasoning_start_time, 1)
                    model_used = current_app.config['LLM_REASONING_MODEL']
                else:
                    process_time = 0
            ...
            is_error = False
            if 'multimodal' in current_app.modules and current_app.modules['multimodal'].available:
                file_size = int((len(file_data) * 3) / 4) if file_data else 0
                is_valid, error = current_app.modules['multimodal'].validate_image(file_data, file_type, file_name, file_size)
                if is_valid:
                    bot_reply, error = current_app.modules['multimodal'].process_image_with_text(file_data, message_text, current_time_str)
            ...
            completion_time_for_db = get_current_time_in_timezone_for_db()
            from .db import save_message
            save_message(session_id, 'assistant', bot_reply, model_name=current_app.config['LLM_MULTIMODAL_MODEL'] if 'multimodal' in current_app.modules else 'system', response_time=str(process_time))
            return {
                'response': bot_reply,
                'session_id': session_id,
                'model_used': current_app.config['LLM_MULTIMODAL_MODEL'] if 'multimodal' in current_app.modules else 'system',
                'assistant_timestamp': completion_time_for_db,
                'response_time': process_time,
                'is_error': is_error
            }
