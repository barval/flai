...
# FIX: Monkey-patch socket to use standard DNS instead of eventlet's greendns
# This must be done BEFORE importing redis
import eventlet
eventlet.monkey_patch(socket=False)  # Don't patch socket to avoid DNS issues


class RedisRequestQueue:
    def __init__(self, app):
        self.app = app
        # FIX: Use socket connection with explicit host resolution
    ...
    def _test_connection(self):
        """Test Redis connection on startup with retry."""
        import time
        max_retries = 10
        retry_delay = 2
        for attempt in range(max_retries):
            try:
                # Use ping with timeout to avoid hanging
                self.redis.ping()
                self.app.logger.info("Redis connection established successfully")
                return True
                ...
                if attempt < max_retries - 1:
                    ...
                    # Re-initialize connection on retry (in case of DNS/network changes)
                    ...
                    self.app.logger.error(f"Failed to connect to Redis after {max_retries} attempts")
                    raise
        return False
    ...
    def _worker_loop(self):
        ...
        while True:
            try:
                ...
                if queue_time > 300:
                    self.app.logger.warning(
                        f"Task {task['id']} waited too long in queue ({queue_time:.1f}s). Cancelling."
                    )
                ...
                self.app.logger.info(
                    f"RedisRequestQueue: got task {task['id']} from queue for session "
                    f"{task.get('session_id')}, queue time: {queue_time:.1f}s"
                )
                ...
                try:
                    with self.app.app_context():
                        ...
                        self.app.logger.info(
                            f"RedisRequestQueue: task {task['id']} completed successfully "
                            f"for session {task.get('session_id')}"
                        )
                        if 'user_id' in task:
                            emit_queue_status(task['user_id'], {
                                'type': 'completed',
                    ...
                    self.app.logger.error(
                        f"RedisRequestQueue: error processing task {task['id']}: {str(e)}"
                    )
                    self.redis.hset(self.results_key, task['id'], pickle.dumps({
                        'status': 'error',
                        'error': str(e),
                ...
                self.app.logger.error(f"RedisRequestQueue: Redis connection error: {str(e)}")
                time.sleep(5)
                try:
                    self.redis.ping()
                    self.app.logger.info("Redis reconnection successful")
                except:
                    self.app.logger.error("Redis reconnection failed")
            except Exception as e:
    ...
    def add_request(self, user_id: str, session_id: str, request_data: Dict[str, Any],
        ...
        self.app.logger.info(
            f"RedisRequestQueue.add_request: adding task {request_id} for session "
            f"{session_id} at {timestamp}"
        )
    ...
    def add_index_task(self, user_id: str, doc_id: str, file_path: str, lang: str = 'ru') -> str:
        ...
        self.app.logger.info(
            f"RedisRequestQueue.add_index_task: adding task {request_id} for document {doc_id}"
        )
        self.redis.rpush(self.queue_key, pickle.dumps(task))
        if 'user_id' in task:
            emit_queue_status(user_id, {
    ...
    def get_user_queue_counts(self, user_id: str) -> Tuple[int, int]:
        try:
            ...
            self.app.logger.error(f"Error getting queue counts: {e}")
            return 0, 0
    ...
    def _process_request(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if request_type == 'text':
            ...
            if action_type == 'reasoning':
                ...
                self.app.logger.info(
                    f"RAG returned no answer, falling back to reasoning model for query: {query[:50]}..."
                )
            ...
            completion_time_for_db = get_current_time_in_timezone_for_db(self.app)
            if final_response:
                message_id = save_message(session_id, 'assistant', final_response,
                                          model_name=model_used, response_time=str(process_time))
                return {
            ...
            completion_time_for_db = get_current_time_in_timezone_for_db(self.app)
            mm_model_name = self._get_model_name('multimodal') or 'unknown'
            message_id = save_message(session_id, 'assistant', bot_reply,
                                      model_name=mm_model_name, response_time=str(process_time))
            return {
                'response': bot_reply,
                'session_id': session_id,
                'model_used': mm_model_name,
    ...
    def _process_index_task(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if success:
                ...
                update_document_index_status(doc_id, INDEX_STATUS_INDEXED,
                                             indexed_at=indexed_at, embedding_model=embedding_model)
    ...
    def _process_reindex_all_task(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.app.logger.info(
            f"Reindex all completed. Total: {total}, Success: {success_count}, Failed: {fail_count}"
        )
    ...
    def get_user_requests_status(self, user_id: str, lang: str = 'ru') -> Dict[str, Any]:
        ...
        try:
            ...
            self.app.logger.error(f"Error getting user requests status: {e}")
        return result

    def _format_request_info(self, task: Dict[str, Any], lang: str = 'ru') -> Dict[str, Any]:
        type_icons = {
            'text': '💬', 'image': '🎨', 'camera': '📷',
            'reasoning': '🧠', 'audio': '🎤', 'index_document': '📄'
        }
        return {
            'id': task['id'],
            'session_id': task.get('session_id'),
    ...
    def check_result(self, request_id: str) -> Optional[Dict[str, Any]]:
        try:
            ...
            self.app.logger.error(f"Error checking result: {e}")
        return None