        ...
        results.append(
            {
                ...
                "text": chunk.get("text", ""),
                "score": score,
                "metadata": {k: v for k, v in chunk.items() if k not in ("text", "doc_id")},
            }
        )

    return jsonify({"query": query, "user_id": user_id, "chunks_found": len(chunks), "results": results})
...
def debug_health():
    ...
    try:
        ...
        resp = requests.get(
            f"{current_app.config.get('LLAMACPP_URL', 'http://flai-llamacpp:8033')}/v1/models", timeout=2
        )
...
def list_documents():
    ...
    try:
        ...
        c = conn.cursor()
        c.execute(
            "SELECT id, filename, file_path, indexed_at, file_size FROM documents WHERE user_id=%s ORDER BY indexed_at",
            (user_id,),
        )
        docs = c.fetchall()
        conn.close()

        result = {"user_id": user_id, "documents_count": len(docs), "documents": []}
        for doc in docs:
            d = (
                dict(doc)
                if hasattr(doc, "keys")
                else {"id": doc[0], "filename": doc[1], "file_path": doc[2], "indexed_at": doc[3], "file_size": doc[4]}
            )
        ...
        return jsonify(
            {
                ...
                "found": len(results),
                "results": [
                    {"doc_id": r.payload.get("doc_id", ""), "score": r.score, "text": r.payload.get("text", "")[:100]}
                    for r in results
                ],
            }
        )
