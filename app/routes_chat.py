...
def chat():
    ...
    if not session.get('current_session'):
        ...
        if last_id and any(s['id'] == last_id for s in sessions):
            ...
            sessions = db.get_user_sessions(user_id)
    # Default active view is 'sessions'
...
def api_upload_document():
    ...
    # Ensure documents_folder is absolute path
    if not os.path.isabs(documents_folder):
        documents_folder = os.path.join(current_app.root_path, '..', documents_folder)
    ...
    # Store relative path from documents_folder
    relative_path = os.path.join(session['login'], f"{doc_id}_{filename}")
    db.save_document(
        session['login'],
        doc_id,
        filename,
