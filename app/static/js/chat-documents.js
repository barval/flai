...
function switchView(view) {
    ...
    // On mobile, if clicking already active tab, toggle collapse
    if (view === currentView && isMobile) {
        if (sidebar) {
    ...
    // On mobile, ensure sidebar is expanded when switching tabs
    if (isMobile && sidebar) {
        sidebar.classList.remove('collapsed');
        const login = window.CURRENT_USER_LOGIN;
        if (login) {
            localStorage.setItem(`sidebar_collapsed_${login}`, 'false');
        }
    }
...
function attachDocumentEventHandlers() {
    document.querySelectorAll('.document-item').forEach(el => {
        el.addEventListener('click', function(e) {
            ...
            // Only select/highlight the document, no download action
...
function uploadDocument(file) {
    ...
    .then(data => {
        if (data.status === 'ok') {
            alert(t('document_uploaded').replace('XXX', file.name));
            loadDocuments();
        } else {
