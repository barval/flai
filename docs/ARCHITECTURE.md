...
> **v9.2 change:** Reasoning task retry-on-empty in `app/queue.py:_process_reasoning_task()`. When the reasoning model (gpt-oss-20b) returns only thinking blocks (empty output after `_strip_thinking_tags()`), the task is retried once instead of immediately showing «No response from reasoning model». The model is already loaded on retry (first attempt loads it just-in-time), so the retry is fast. No retry on task cancellation or on genuine LLM error strings. Additionally: (1) `ensure_vram_for()` returns `True` immediately when the needed model is already loaded — no unload/reload cycle, (2) `chat_with_image_stream()` in `llamacpp_client.py` passes `ensure_vram=False` to `chat_stream()` — eliminates redundant VRAM check when queue.py already guaranteed VRAM, (3) streaming token rendering throttled to 120 ms in `onStreamToken()` and sessionStorage writes to 500 ms (capped at 16 KB), (4) single-quoted JSON tool calls parsed via `ast.literal_eval()` fallback in `_try_parse_text_tool_call()`, (5) `RagModule` gains `check_availability()` for per-request reconnection and 3× startup retry, (6) `search()`→`query_points()` migration for qdrant-client 1.19.0, (7) SD wrapper selects optimal `start_offload_level` based on VRAM budget (model weights + VAE decode buffer), (8) camera section injected as `## 4` (was `## 5`, conflicting with document search), (9) router prompt fixes for person-name queries (`[-RAG-]` instead of `[-SEARCH-]`), (10) SLM recall merged into single call with 15s semantic timeout.
