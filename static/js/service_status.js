...
class ServiceStatusMonitor {
        ...
        if (allHealthy) {
            ...
            for (const [name, status] of Object.entries(statuses)) {
                if (status.health !== 'healthy') {
                    let displayName = name === 'automatic1111' ? 'Automatic1111' : name === 'camera_api' ? 'Камеры' : 'Ollama';
    ...
    getLocationText(location) {
        const texts = { 'local': '🏠 Локальный', 'remote': '🌐 Удалённый', 'unknown': '❓ Неизвестно' };
        return texts[location] || location;
    }
    
    getHealthText(health) {
        const texts = { 'healthy': 'Работает', 'degraded': 'Проблемы', 'down': 'Недоступен', 'unknown': 'Статус неизвестен' };
