...
def get_session():
    ...
    response = session.post(
        f"{BASE_URL}/login",
        data={"login": TEST_USER, "password": TEST_PASS, "csrf_token": csrf_token},
...
def test_chat(session, prompt="Hello! How are you?"):
    ...
    for i in range(ITERATIONS):
        start = time.time()
        resp = session.post(f"{BASE_URL}/api/chat", json={"message": prompt, "session_id": None}, timeout=60)
...
def test_reasoning(session):
    ...
    for i in range(ITERATIONS):
        start = time.time()
        resp = session.post(
            f"{BASE_URL}/api/chat", json={"message": prompt, "session_id": None, "use_reasoning": True}, timeout=300
        )
        elapsed = time.time() - start
        times.append(elapsed)
        print(f"    Iteration {i + 1}: {elapsed:.2f}s - Status: {resp.status_code}")
...
def test_image_generation(session, prompt="a cat sitting on a windowsill"):
    ...
    for i in range(ITERATIONS):
        start = time.time()
        resp = session.post(f"{BASE_URL}/api/image/generate", json={"prompt": prompt}, timeout=600)
        elapsed = time.time() - start
        times.append(elapsed)
        print(f"    Iteration {i + 1}: {elapsed:.2f}s - Status: {resp.status_code}")
    return times
...
def test_image_analysis(session):
    ...
    for i in range(ITERATIONS):
        start = time.time()
        resp = session.post(
            f"{BASE_URL}/api/multimodal",
            json={"image": png_data, "message": "What is shown in this picture?"},
            timeout=120,
        )
        elapsed = time.time() - start
        times.append(elapsed)
...
def test_image_editing(session):
    ...
    for i in range(ITERATIONS):
        start = time.time()
        resp = session.post(
            f"{BASE_URL}/api/image/edit",
            json={"image": png_data, "prompt": "make the background brighter"},
            timeout=900,
        )
        elapsed = time.time() - start
        times.append(elapsed)
...
def test_tts(session, text="Hello! This is a text-to-speech test."):
    ...
    for i in range(ITERATIONS):
        start = time.time()
        resp = session.post(f"{BASE_URL}/api/tts", json={"text": text, "voice_gender": "male"}, timeout=30)
        elapsed = time.time() - start
        times.append(elapsed)
        print(f"    Iteration {i + 1}: {elapsed:.2f}s - Status: {resp.status_code}")
    return times
...
def test_asr(session):
    ...
    for i in range(ITERATIONS):
        start = time.time()
        resp = session.post(f"{BASE_URL}/api/asr", json={"audio": wav_data}, timeout=60)
        elapsed = time.time() - start
        times.append(elapsed)
        print(f"    Iteration {i + 1}: {elapsed:.2f}s - Status: {resp.status_code}")
    return times
...
def test_rag(session):
    ...
    for i in range(ITERATIONS):
        start = time.time()
        resp = session.post(f"{BASE_URL}/api/rag", json={"query": "test", "session_id": None}, timeout=30)
        elapsed = time.time() - start
        times.append(elapsed)
        print(f"    Iteration {i + 1}: {elapsed:.2f}s - Status: {resp.status_code}")
    return times
