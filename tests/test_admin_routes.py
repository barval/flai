...
class TestAdminAuth:
    ...
    def test_admin_panel_requires_auth(self, client):
        ...
        # admin_required decorator returns 403 for unauthenticated users
...
class TestAdminUsers:
    ...
    def test_add_duplicate_user(self, admin_client, test_app):
        ...
        response = admin_client.post(
            "/admin/api/users",
            data=json.dumps({"login": "existinguser", "password": "newpass", "name": "Duplicate User"}),
            content_type="application/json",
        )
        assert response.status_code == 400
    ...
    def test_update_user(self, admin_client, test_app):
        ...
        response = admin_client.put(
            "/admin/api/users/updatetest",
            data=json.dumps({"name": "Updated Name", "service_class": 1}),
            content_type="application/json",
        )
        assert response.status_code == 200
        data = response.get_json()
    ...
    def test_change_password(self, admin_client, test_app):
        ...
        response = admin_client.put(
            "/admin/api/users/pwdtest/password",
    ...
    def test_change_password_empty(self, admin_client):
        ...
        response = admin_client.put(
            "/admin/api/users/admin/password", data=json.dumps({"new_password": ""}), content_type="application/json"
        )
        assert response.status_code == 400
