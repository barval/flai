...
class TestDocumentsCRUD:
    ...
    def test_upload_document_pdf(self, mock_magic, authenticated_client):
        ...
        response = authenticated_client.post(
            "/api/documents/upload", data={"file": (test_file, "test.pdf")}, content_type="multipart/form-data"
        )
        assert response.status_code == 200
        data = response.get_json()
    ...
    def test_upload_document_docx(self, mock_magic, authenticated_client):
        ...
        response = authenticated_client.post(
            "/api/documents/upload", data={"file": (test_file, "test.docx")}, content_type="multipart/form-data"
        )
        assert response.status_code == 200
    ...
    def test_upload_document_txt(self, mock_magic, authenticated_client):
        ...
        response = authenticated_client.post(
            "/api/documents/upload", data={"file": (test_file, "test.txt")}, content_type="multipart/form-data"
        )
        assert response.status_code == 200

    def test_upload_document_no_file(self, authenticated_client):
        """Test uploading document without file."""
        response = authenticated_client.post("/api/documents/upload", data={}, content_type="multipart/form-data")
    ...
    def test_upload_document_unsupported_type(self, mock_magic, authenticated_client):
        ...
        response = authenticated_client.post(
            "/api/documents/upload", data={"file": (test_file, "test.exe")}, content_type="multipart/form-data"
        )
        assert response.status_code == 400
...
class TestDocumentValidation:
    ...
    def test_file_size_limit(self, mock_magic, authenticated_client):
        ...
        response = authenticated_client.post(
            "/api/documents/upload", data={"file": (test_file, "large.txt")}, content_type="multipart/form-data"
        )
