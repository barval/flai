...
class TestSaveAndRespondOmitsHeaderFieldsOnError:
    ...
    def test_error_result_omits_response_time(self, q):
        with (
            patch("app.queue.save_message", return_value=42),
            patch.object(q, "_build_success_response", return_value=_build_success_dict_stub()),
        ):
        ...
        assert "response_time" not in result, "is_error=True must strip response_time to prevent ⏱️ in header"
    ...
    def test_error_result_omits_response_style(self, q):
        ...
        assert "response_style" not in result, "is_error=True must strip response_style to prevent 🤖 in header"
    ...
    def test_error_result_omits_completion_tokens(self, q):
        ...
        assert "completion_tokens" not in result, "is_error=True must strip completion_tokens to prevent 🚀 in header"
