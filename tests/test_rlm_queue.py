...
@pytest.mark.unit
def test_process_rlm_task_uses_recognized_text_for_scanned_pdf(tmp_path):
    q = RedisRequestQueue.__new__(RedisRequestQueue)
    q._publish_stream_event = lambda task, event_type, extra=None: None
    q._is_task_cancelled = Mock(return_value=False)
    q._get_model_name = Mock(return_value="reasoning")
    ...
    app = Mock()
    app.modules = {"base": Mock()}
    app.modules["base"]._ = Mock(return_value="⚠️ no text")
    app.config = {"DOCUMENTS_FOLDER": str(tmp_path)}
    q.app = app

    pdf_path = tmp_path / "user" / "dog_prod.pdf"
    pdf_path.parent.mkdir()
    pdf_path.write_bytes(b"%PDF-1.4 scanned document")
    recognized_path = pdf_path.with_suffix(".recognized_text")
    recognized_path.write_text("OCR text: apartment sale agreement at Kurchatov street.", encoding="utf-8")

    task = {
        "id": "rlm-selected-scanned-pdf",
        "data": {"type": "rlm_analysis", "text": "What is known?", "doc_ids": ["pdf-1"]},
        "session_id": "s1",
        "user_id": "u1",
        "lang": "en",
    }
    ...
    with (
        patch("modules.rlm.RlmModule") as mock_rlm_module,
        patch("app.db.get_document", return_value={"file_path": "user/dog_prod.pdf", "filename": "dog_prod.pdf"}),
        patch("app.utils.extract_text_from_file", return_value=None),
        patch("app.resource_manager.get_resource_manager", return_value=rm),
    ):
        mock_rlm_module.return_value.run = run_mock
    ...
    assert result["status"] == "ok"
    assert run_mock.call_args.kwargs["corpus"] == {
        "dog_prod.pdf": "OCR text: apartment sale agreement at Kurchatov street."
    }
